...
class AnswerAwareTokenizer:
    ...
    def all_word_positions(self, encoding, index):
        ...
        offsets = [
            position
            for position, (last_word_number, current_word_number) in enumerate(
                zip([-1] + words, words)
            )
    ...
    def characters_to_tokens(self, text, answers, encoding, index, offset, endpos):
        ...
        tokens_endpos = (
            tokens_endpos if tokens_endpos is not None else len(encoding.tokens(index))
        )
    ...
    def tokens_to_answer(self, encoding, index, text, tokens_offset, tokens_endpos):
        ...
        char_offset = encoding.word_to_chars(
            index, encoding.token_to_word(index, tokens_offset)
        ).start

        try:
            char_next_offset = encoding.word_to_chars(
                index, encoding.token_to_word(index, tokens_endpos)
            ).start
            char_endpos = char_next_offset
        except:
            char_endpos = encoding.word_to_chars(
                index, encoding.token_to_word(index, tokens_endpos - 1)
            ).end
...
class TokenizationObject:
    def __init__(
        ...
        self.tok = tokenizer
        self.questions = (
            questions if len(questions) == len(passages) else questions * len(passages)
        )
        self.passages = passages
        self.answers = answers

        self.encoding, self.query_lengths = self._encode()
        self.passages_only_encoding, self.candidates, self.candidates_list = (
            self._candidize()
        )
    ...
    def _candidize(self):
        ...
        all_candidates = [
            self.tok.get_all_candidates(encoding, index)
            for index in range(len(self.passages))
        ]

        bsize, maxcands = len(self.passages), max(map(len, all_candidates))
        all_candidates = [
            cands + [(-1, -1)] * (maxcands - len(cands)) for cands in all_candidates
        ]

        candidates = torch.tensor(all_candidates)
        assert candidates.size() == (bsize, maxcands, 2), (
            candidates.size(),
            (bsize, maxcands, 2),
